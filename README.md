...
- **Syntax highlighting in the text view** — the text view **auto‑detects the language** (by extension, else a content sniff) and colours it: JavaScript, PowerShell, VBScript, Python, Lua, shell, batch, PHP, C‑like, JSON, XML/HTML, Markdown, and **YARA** (hex byte‑patterns, regexes, and `$`/`#`/`@`/`!` string identifiers included) — handy for reading dropped scripts, configs, and your rule library. A **Syntax** picker lets you override the guess per file (e.g. force VBScript on a decoded `.vbe` saved as `.txt`). Built‑in and dependency‑free
...
- **Type‑to‑edit** — overwrite bytes by typing hex or ASCII in the grid, or edit the file as text; insert/delete/resize; all with unlimited undo/redo
