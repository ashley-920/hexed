...
- **Syntax highlighting in the text view** — the text view **auto‑detects the language** (by extension, else a content sniff) and colours it: JavaScript, PowerShell, VBScript, Python, shell, batch, PHP, C‑like, JSON, XML/HTML, Markdown, and **YARA** (hex byte‑patterns, regexes, and `$`/`#`/`@`/`!` string identifiers included) — handy for reading dropped scripts, configs, and your rule library. Built‑in and dependency‑free
